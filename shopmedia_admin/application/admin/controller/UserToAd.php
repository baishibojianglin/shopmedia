...
/**
 * admin模块用户（广告主业务员）控制器类
 * Class UserToAd
 * @package app\admin\controller
 */
class UserToAd extends Base
{
    /**
     * 显示用户资源列表
    ...
    public function index()
    {
        ...
        if (request()->isGet()) {
            ...
            try {
                $data = model('User')->getUserToAd($map, $this->size);
            } catch (\Exception $e) {
                return show(config('code.error'), '网络忙，请重试', '', 500); // $e->getMessage()
            }
            $status = config('code.status');
            foreach ($data as $key => $value) {
                $data[$key]['status'] = $value['status'] == config('code.status_enable') ? $value['uta_status'] : config('code.status_disable'); // 状态
    ...
    public function read($id)
    {
        ...
        if (request()->isGet()) {
            ...
            // 获取设备合作业务员信息
            try {
                $data = Db::name('user_to_ad')->alias('uta')->field('uta.user_id, uta.user_type, uta.money, uta.income, uta.cash, uta.status, uta.parent_comm_ratio, uta.auth_son_ratio, uta.comm_ratio, u.user_name, u.user_type user_types, u.phone, u.avatar')->join('__USER__ u', 'uta.user_id = u.user_id', 'INNER')->where(['uta.user_id' => $id, 'uta.user_type' => ['in', $user['user_type']]])->find();
            } catch (\Exception $e) {
                return show(config('code.error'), '网络忙，请重试', '', 500);
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $result = Db::name('user_to_ad')->where(['user_id' => $id])->update($data);
        } catch(\Exception $e) {
