...
class AuthRuleMenus extends Base
{
    ...
    protected function getAuthRuleMenus($uid, $type =2)
    {
        ...
        if (2 == $type && Session::has('_auth_list_' . $uid . $t, config('app.session_admin_scope'))) {
            return Session::get('_auth_list_' . $uid . $t, config('app.session_admin_scope'));
        }
        //读取用户所属用户组
        ...
        if (2 == $type) {
            //规则列表结果保存到session
            Session::set('_auth_list_' . $uid . $t, $authList, config('app.session_admin_scope'));
        }
