    ...
    public function index()
    {
        ...
        if (request()->isGet()) {
            ...
            foreach ($data as $key => $value) {
                $data[$key]['status'] = $value['status'] == config('code.status_enable') ? $value['to_partner_status'] : config('code.status_disable'); // 状态
                $data[$key]['status_msg'] = $status[$value['to_partner_status']]; // 定义状态信息
