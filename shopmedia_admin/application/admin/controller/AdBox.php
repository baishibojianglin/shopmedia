...
class AdBox extends Base
{
	...
	public function index()
	{
		...
		if (request()->isGet()) {
			...
			if (isset($param['status']) && $param['status'] != null) { // 状态
				$map['ab.status'] = intval($param['status']);
			}
			...
			// 获取广告框列表数据
			try {
				$data = model('AdBox')->getAdBox($map, (int)$this->size);
			} catch (\Exception $e) {
	...
	/**
	 * 新增或更新广告框（弃用，已由新建save()、更新update()代替）
	 * @return \think\Response
	 * @throws ApiException
	 */
