		...
		<el-card class="main-card">
			<div slot="header" class="clearfix">
				<el-row :gutter="20" type="flex" justify="space-between">
					<el-col :span="6"><span>店铺端业务员</span></el-col>
					<el-col :span="6">
						<!-- 查询 s -->
...
<script>
	export default {
		...
		methods: {
			...
			getUserList() {
				let self = this;
				this.$axios.get(this.$url + 'user_to_shop', {
					params: {
			...
			deleteUser(scope) {
				this.$confirm('此操作将永久删除该用户, 是否继续?', '删除', {
					...
					// 调用删除接口
					let self = this;
					this.$axios.delete(this.$url + 'user_to_shop/' + scope.row.user_id)
						.then(function(res) {
							// 移除元素
