	     ...
	     <el-row>
			  <el-col :span="15" style="margin-left: 50px;">
					<el-form  ref="ruleForm" :model="ruleForm"   label-width="0px">
				       ...
				       <p><span style="color:#f00;">*</span> 选择主营商品种类：</p>
					   <el-tree  ref="tree"  :default-expanded-keys="opendata" :default-checked-keys="checkdata"  empty-text="" show-checkbox node-key="cate_id" :props="props" :load="loadNode" lazy></el-tree> 
 ...
 <script>
   export default {
	 ...
	 mounted(){
		 ...
		 this.companyid=company.company_id; //获取登录账号所属的经销商id
	 },
		  ...
		  submitForm(formName) {
            ...
            self.ruleForm.salecate='';
			 ...
			 this.$refs.tree.getCheckedNodes().forEach((value,index)=>{
			 	self.ruleForm.salecate=self.ruleForm.salecate+value.cate_id+'|';
			 })
			 //去除最后一个符号"|"
			 self.ruleForm.salecate=self.ruleForm.salecate.slice(0,-1);
			 //如果存在半选，设置半选分割点
			 if(this.$refs.tree.getHalfCheckedNodes().length>0){
				 self.ruleForm.salecate=self.ruleForm.salecate+',';
				 //获取半选的数据
			...
			this.$refs[formName].validate((valid) => {
			  if (valid) {
				this.$axios.post(this.$url+'shopcateinsert',{
				   data:this.ruleForm
				}).then(function(res){
