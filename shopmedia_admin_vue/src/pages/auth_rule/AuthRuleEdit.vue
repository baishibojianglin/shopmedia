<template>
	<div class="auth_rule_edit">
		<el-card class="main-card">
			<div slot="header" class="clearfix">
				<el-row :gutter="20" type="flex" justify="space-between">
					<el-col :span="6"><span>编辑权限规则</span></el-col>
					<el-col :span="3">
...
<script>
	export default {
		...
		methods: {
			...
			getAuthRule() {
				let self = this;
				this.$axios.get(this.$url + 'auth_rule/' + this.form.id, {
					headers: {
			...
			submitForm(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.put(this.$url + 'auth_rule/' + this.form.id, {
							// 参数
							name: this.form.name,
