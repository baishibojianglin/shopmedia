		...
		<el-card class="main-card">
			<el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="150px">
				...
				<el-form-item label="设备图片(5张以内)" prop="url_image" class="idcard">
					...
					<el-upload :class="{hide:hideUpload[0]}" list-type="picture-card" :action="this.$url+'upload?name=image'" :limit="5"
					 :on-success="function (res,file,fileList) { return returnUrl(res,file,fileList,'url_image',0)}" :on-change="function (file,fileList) { return delePlusButton(file,fileList,5,0)}"
					 :on-remove="function (file,fileList) { return handleRemove(file,fileList,0,5,'url_image')}" :on-preview="handlePictureCardPreview"
					 name='image'>
				...
				<el-form-item label="排除广告类型" prop="remove_ad_cate">
					<el-select v-model="ruleForm.remove_ad_cate" placeholder="请选择" disabled>
						<el-option v-for="item in shop_options" :key="item.value" :label="item.cate_name" :value="item.cate">
						</el-option>
					</el-select>
		...
		methods: {
			...
			submitForm(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.post(this.$url + 'device', {
							data: this.ruleForm
						}).then(function(res) {
