		...
		<view class="uni-page-body mb">
			...
			<view class="content-con">
				...
				<view class="content">
					...
					<input class="content-right sign-border"  placeholder="请填写姓名" v-model="user_name" :disabled="inputDisabled" />
				</view>
...
<script>
	...
	export default {
		...
		onLoad(event) {
			this.csPhone = typeof(event.cs_phone) != 'undefined' ? event.cs_phone : this.csPhone;
		...
		methods: {
			...
			buttonClick(e) {
				let self = this;
				if (typeof(this.partnerOrder.order_status) != 'undefined' && this.partnerOrder.order_status == 0) {  // 已签约，未付款
					uni.showModal({
					...
					success: (res) => {
						if (res.data.status == 1) {
							...
							if (self.partnerOrder.party_b_signature) {
								...
								// 已签约，未付款
								if (self.partnerOrder.order_status == 0) {
									// GoodsNav button-group属性重新赋值
									self.buttonGroup[0].text = '已签约，确认付款';
									self.buttonGroup[0].backgroundColor = '#dd524d';
								}
							}
						}
					},
