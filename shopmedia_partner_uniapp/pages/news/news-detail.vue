<template>
	<view>
		...
		<view class="article-meta">
			...
			<text class="article-time">{{banner.publish_time}}</text>
		</view>
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap(event) {
			this.$common.actionSheetTap();return;
		...
		methods: {
			getDetail() {
				uni.request({
					url: this.$serverUrl + 'api/news/' + this.banner.news_id,
					header: {
						'commonheader': this.$store.state.commonheader
					},
					success: (data) => {
						if (data.statusCode == 200) {
							var htmlString = data.data.data.content.replace(/\\/g, "").replace(/<img/g, "<img style=\"display:none;\"");
