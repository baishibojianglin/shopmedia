<template>
	<view class="content">
		<view v-if="hasLogin" class="uni-flex uni-row"><!-- login_info.has_login -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			getUserInfo() {
				let self = this
				if (this.hasLogin) { // this.login_info.has_login
					uni.request({
						url: this.$serverUrl + 'api/user/' + this.userInfo.user_id, // this.login_info.user_id
						header: {
							'sign': common.sign(), // 验签
							...
							'access-user-token': this.userInfo.token // this.login_info.user_info.token
						},
						method: 'GET',
